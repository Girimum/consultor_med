...
public class DaoSecretaria {    
    ...
    public void Salvar(Beans mod){
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("insert into secretaria (nome, cpf, rg, telefone, endereco, sexo, senha) values(?, ?, ?, ?, ?, ?, ?)");
    ...
    public void Editar(Beans mod){
        conex.conexao();
            try {
                PreparedStatement pst = conex.con.prepareStatement("UPDATE medicos set  nome=?, cpf=?, rg=?, telefone=?, endereco=?, sexo=?, senha=? where codigo_secretaria");
                pst.setString(2, mod.getNome());
                pst.setString(3, mod.getCpf ());
                pst.setString(4, mod.getRg ());
                pst.setString(5, mod.getTelefone());
